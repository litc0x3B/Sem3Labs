...
TEST(dynamicArraySequenceTest, concat)
{
    ...
    Sequence<int> *dynamicArray1 = new DynamicArraySequence(arr1, ARRAY_SIZE(arr1));
    Sequence<int> *dynamicArray2 = new DynamicArraySequence(arr2, ARRAY_SIZE(arr2));
...
TEST(dynamicArraySequenceTest, subsequence)
{
    ...
    containersEqual(*array2, expected, ARRAY_SIZE(expected));
    EXPECT_EQ(array2->GetSize(), ARRAY_SIZE(expected));
...
class TestClass
{
   ...
   public:
    ...
    bool operator!=(const TestClass &obj) {return !(*this == obj);}
